...
#include "../brick/util/BrickRegistrar.h"
...
Brickblock::~Brickblock()
{
	...
	BrickRegistrar::cleanupInstance();
}
...
void Brickblock::init(GLint argc, GLint **argv)
{
	if (!mIsInitialized)
	{
		...
		BBLogger::logTrace(CLASS_NAME, "Registering Brick Registrar...");
		BrickRegistrar::initInstance();

		BBLogger::logTrace(CLASS_NAME, "Loading Bricks...");
		BrickList::createBricks();

		BBLogger::logTrace(CLASS_NAME, "Registering Bricks...");
		BrickList::registerBricks();
