...
void Camera::update(const GLdouble& deltaTime)
{
	...
	GLboolean rotateRight, rotateLeft;
	if (GameSettings::getRotateLeftKey().isPressed())
	{
		rotateRight = GL_TRUE;
	}

	if (GameSettings::getRotateRightKey().isPressed())
	{
		rotateLeft = GL_TRUE;
	}

	GLfloat rotationSpeed = 2.0f * deltaTime;
	mTransform->addRotation(0.0f, -rotationSpeed * GameSettings::getRotateRightKey().isPressed() + rotationSpeed * GameSettings::getRotateLeftKey().isPressed(), 0.0f);
